...
function loadAll() {
  ...
  for (let i = 1; i < memberData.length; i++) {
    const [id, name, classId, deptsRaw, phone] = memberData[i];
    if (!id) continue;
    const depts = deptsRaw ? deptsRaw.split(',').filter(Boolean) : [];
    members.push({id: String(id), name, classId, depts, phone: phone||''});
  }
...
function saveMember(member) {
  ...
  for (let i = 1; i < existing.length; i++) {
    if (String(existing[i][0]) === String(member.id)) {
      memberSheet.getRange(i+1, 1, 1, 5).setValues([
        [member.id, member.name, member.classId, (member.depts||[]).join(','), member.phone||'']
      ]);
      return {success: true, action: 'updated'};
    }
  }
  memberSheet.appendRow([member.id, member.name, member.classId, (member.depts||[]).join(','), member.phone||'']);
...
function ensureSheetsExist() {
  ...
  getOrCreate(SHEETS.MEMBERS,   ['id','name','classId','departments','phone']);
